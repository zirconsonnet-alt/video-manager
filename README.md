...
3. 如果目标目录里已经有同名 `mp4`，勾选“不重复下载”后会直接跳过，不再生成 `(2)`、`(3)` 这种重复文件。
4. 可选填写“视频命名”；批量任务或展开合集时建议留空，让程序自动使用单集标题命名，不再把合集标题拼进文件名。
5. 选择视频保存目录。
6. 点击“加入下载队列”。
7. 程序会按队列顺序逐个下载，你可以继续追加新的链接。
...
- 勾选“不重复下载”后，如果检测到目标目录里已有同名文件，或当前队列里已存在同名任务，会直接标记为“已跳过”。
